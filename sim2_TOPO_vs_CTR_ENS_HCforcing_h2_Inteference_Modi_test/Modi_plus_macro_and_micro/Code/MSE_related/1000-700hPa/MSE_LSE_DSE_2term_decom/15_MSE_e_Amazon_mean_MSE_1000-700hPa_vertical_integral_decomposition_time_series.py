...
for i_case in range(len(cases)):
    for i in range(len(file_names)):
        ds = xr.open_dataset(data_path+file_names[i]+'1000-700.nc', decode_times=False)
    ...
    plt.xlabel('time, hr')
    plt.ylabel('kJ/kg')
    plt.title(cases[i_case]+', Amazon avg, 1000-700hPa')
    plt.grid(True)
...
plt.tight_layout()
#plt.show()
plt.savefig('../Figures/CTR_TOPO_Amazon_mean_MSE_1000-700hPa_decomp.png')
